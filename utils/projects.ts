import { v4 as uuidv4 } from "uuid";
export const projectData = [
  {
    id: uuidv4(),
    url: "https://aiido.co",
    title: "Aiido",
    imgUrl: `/Aiido.webp`,
    description:
      "Plan your wedding stress-free and within budget. Smart budget recommendations and 200+ verified vendors across Nigeria, all in one place.",
    skills: ["Nextjs", "TypeScript", "Tailwindcss"],
  },
  {
    id: uuidv4(),
    url: "https://urltrim.xyz",
    title: "Urltrim",
    imgUrl: `/Ulrtrim.webp`,
    description:
      "Create, share, and track your links with ease. Build your brand with every click using our professional, lightning-fast URL shortening service.",
    skills: ["Tailwindcss", "Javascript", "Nextjs", "TypeScript", "React-query", "Nestjs", "Prisma", "Postgres"],
  },
  {
    id: uuidv4(),
    url: "https://kyshi.co",
    title: "Kyshi",
    imgUrl: `/Kyshi.webp`,
    description:
      "Unlocking Boundless Possibilities in Payments and Revenue in Africa",
    skills: ["CSS", "HTML", "Vue", "Nuxt", "Tailwindcss"],
  },
  {
    id: uuidv4(),
    url: "https://cybersocafrica.com",
    title: "CybersocAfrica",
    imgUrl: `/Cybersoc.webp`,
    description: "This is the official CybersocAfrica website",
    skills: ["CSS", "HTML", "Javascript", "GIT", "bootstrap"],
  },
  {
    id: uuidv4(),
    url: "https://kokua.wiki",
    title: "Kokua - Free Resources",
    imgUrl: `/Kokua.webp`,
    description:
      "A free resources application, very handy for all beginner and professioner developers",
    skills: ["Chakra", "HTML", "Javascript", "Nextjs", "TypeScript", "Strapi"],
  },
  {
    id: uuidv4(),
    url: "https://cyberlabafrica.com",
    title: "Cyberlab",
    imgUrl: `/Cyberlab.webp`,
    description: "This is the official Cyberslab website",
    skills: ["CSS", "HTML", "Javascript", "GIT", "bootstrap"],
  },
  {
    id: uuidv4(),
    url: "https://tunga.io",
    title: "TUNGA",
    imgUrl: `/Tunga.webp`,
    description:
      "Tunga is a staff augmentation firm that works with African software developers. We have helped 150+ companies worldwide with IT staffing and software outsourcing. Add experienced developers to your team within days!",
    skills: ["CSS", "HTML", "Javascript", "Web-components", "GIT"],
  },
  {
    id: uuidv4(),
    url: "https://resumeranker.io/",
    title: "ResumeRanker",
    imgUrl: `/Resumeranker.webp`,
    description: "Elevate your job search with ResumeRanker's AI tools.",
    skills: [
      "CSS",
      "HTML",
      "Javascript",
      "Nextjs",
      "Tailwindcss",
      "TypeScript",
    ],
  },
  {
    id: uuidv4(),
    url: "https://testing.tunga.io",
    title: "Tunga-Testing",
    imgUrl: `/Tunga-testing.webp`,
    description:
      "Tunga's testing platform, for vetting developers to be added to the tunga pool, contains various kinds of tests, ranging from multiple choice questions to code challenges and even personality tests.",
    skills: [
      "CSS",
      "HTML",
      "Javascript",
      "React",
      "GIT",
      "bootstrap",
      "styled-components",
    ],
  },
  {
    id: uuidv4(),
    url: "https://bodsquare.com",
    title: "BodSquare",
    imgUrl: `/Bodsquare.webp`,
    description: "This is the official Bodsquare website",
    skills: ["CSS", "HTML", "Javascript", "React", "ChakraUI", "TypeScript"],
  },
  {
    id: uuidv4(),
    url: "https://www.sendchamp.com/",
    title: "Sendchamp",
    imgUrl: `/Sendchamp.webp`,
    description:
      "Sendchamp is the most seamless messaging platform for growing businesses. Send messages to your customers across WhatsApp, SMS, email, and voice - all with zero coding skills to get started.",
    skills: ["Chakra", "HTML", "Javascript", "React", "TypeScript", "Recoiljs"],
  },
  {
    id: uuidv4(),
    url: "https://anonn.xyz",
    title: "Anonn - Anonymous Chat",
    imgUrl: `/Anonn.webp`,
    description:
      "Anonymously chat with people, they are free to say anything to you since you never get to know who you're chatting with except they tell you",
    skills: ["Tailwindcss", "HTML", "Javascript", "Nextjs", "TypeScript"],
  },
  {
    id: uuidv4(),
    url: "https://damicode.netlify.app",
    title: "Portfolio",
    imgUrl: `/Portfolio.webp`,
    description:
      "Just a portfolio app, that shows all my projects, I bet you're currently viewing it",
    skills: ["CSS", "HTML", "Javascript", "Nextjs", "TypeScript"],
  },
  {
    id: uuidv4(),
    url: "https://dash.bodsquare.com",
    title: "BodSquare WebApp",
    imgUrl: `/Bodsquare-dashboard.webp`,
    description:
      "Bodsquare allows for businesses to follow up with their customers from the moment they contact them until the end, regardless of the platform used.",
    skills: [
      "CSS",
      "HTML",
      "Javascript",
      "TypeScript",
      "React",
      "ChakraUI",
      "Recoiljs",
    ],
  }
];
